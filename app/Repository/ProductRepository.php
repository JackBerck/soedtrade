...
class ProductRepository
{
    ...
    {
        $statement = $this->connection->prepare("INSERT INTO saved_products(user_id, product_id) VALUES (?, ?)");
        $statement->execute([$userId, $productId]);
    }

    public function deleteSavedProduct(int $userId, int $productId): void
    {
        $statement = $this->connection->prepare("DELETE FROM saved_products WHERE user_id = ? AND product_id = ?");
        $statement->execute([$userId, $productId]);
    }

    public function isProductSaved(int $userId, int $productId): bool
    {
        $statement = $this->connection->prepare("SELECT COUNT(*) FROM saved_products WHERE user_id = ? AND product_id = ?");
        $statement->execute([$userId, $productId]);
        return (int)$statement->fetchColumn() > 0;
    }

    public function getSavedProducts(string $user_id): array
    {
        $statement = $this->connection->prepare("
        SELECT p.product_id, p.seller_id, p.name, p.price, p.description, p.condition, p.category, p.created_at, pi.image_name
        FROM products p
        LEFT JOIN product_images pi ON p.product_id = pi.product_id
        JOIN saved_products sp ON p.product_id = sp.product_id
        WHERE sp.user_id = ?
    ");
